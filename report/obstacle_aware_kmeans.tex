...
The algorithm iterates these assignment and update steps to convergence. We seed it with a variant of k-means++ adapted to the same composite distance: the first centroid is drawn uniformly at random, and each subsequent centroid is sampled with probability proportional to $d^2$ in \eqref{eq:composite} from the nearest existing centroid. Every run in this report uses a fixed random seed, so the initializations, cluster sizes, and iteration counts reproduce exactly.
