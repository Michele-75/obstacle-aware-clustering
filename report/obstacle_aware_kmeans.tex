...
This matters whenever a clustering result is going to be used. Suppose we are trying to place firefighter stations at central locations for the fire hotspots around a lake. The center of a cluster, in straight-line terms, can sit on the opposite side of the water from many of the fires it nominally represents. The same issue appears in placing disaster response teams near sources of danger, in studying species separated by a physical barrier, in urban planning around major roadways, and in dispersing resources efficiently across a region. In each case, the clustering should reflect distances along traversable paths rather than across barriers. Standard k-means cannot make that distinction. Two points on opposite sides of a lake look as close as two points on the same shoreline.
...
Obstacle-aware k-means keeps the structure of standard k-means -- alternate between assigning points to the nearest centroid, which partitions the
fires into clusters $C_1, \dots, C_k$, and updating centroids -- but changes the distance used in the assignment step. For a point $\mathbf{x} = (\text{geo}, s)$ and a centroid $\boldsymbol{\mu} = (\text{geo}_\mu, s_\mu)$,
\begin{equation}
...
\]
but the arc-length part cannot be averaged directly without misbehaving at the seam. Instead we project each member onto the boundary, average those projected
positions, and project the average back onto the boundary to get $s_{\mu_c}$:
\[
s_{\mu_c} = s\!\left( \frac{1}{|C_c|} \sum_{i \in C_c} \big( x(t_i),\, y(t_i) \big) \right),
\]
where $t_i$ is the parameter at which member $i$ projects onto the boundary, so
$\big(x(t_i), y(t_i)\big)$ is that member's position on the shoreline, and $s(\cdot)$
projects its argument back onto the boundary and returns the normalized arc length.
The centroid's shoreline position is the arc length of this reprojected average of
its members' shoreline positions, consistent with how points get their $s$ values in
the first place.

The algorithm iterates these assignment and update steps to convergence. We seed it with a variant of k-means++ adapted to the same composite distance: the first centroid is drawn uniformly at random, and each subsequent centroid is sampled with probability proportional to $d^2$ in \eqref{eq:composite} from the nearest existing centroid. All runs in this report use fixed random seeds, so the initializations, cluster sizes, and iteration counts reproduce exactly.
...
\caption{Standard k-means (top) misassigns one point at the right tip of the ellipse, where straight-line distance pulls a south-east point into the north-east cluster. Obstacle-aware k-means (bottom) recovers the correct grouping. The two algorithms both use k-means++ and $k=3$ on the same data; the obstacle-aware version adds the $s$ term to the distance.}
...
Lake Tahoe sits on the California-Nevada border in the Sierra Nevada, a single roughly oval basin about 35 km long and 19 km wide. Tahoe makes a good test case because it is one closed body of water with a compact shape and a well-defined regulatory boundary around it for selecting fires. The analysis runs at two scales: the basin scale uses every fire inside the Tahoe basin boundary (1,068 fires over 29 years), and the near-shore scale uses only the fires within 1 km of the shoreline.
...
Each of the 1,068 fires is projected onto the boundary spline using the procedure from Section~\ref{sec:method}, yielding both its spline parameter $t$ and its arc-length position $s \in [0,1]$. The full range of $s$ is covered, $0.007$ to $0.998$, so the fires touch essentially every stretch of shoreline (see Figure~\ref{fig:tahoe-s}).
...
The number of clusters $k$ is held fixed across both lakes for a controlled comparison. The value $k = 4$ comes from the elbow method run on Tahoe's basin coordinates, sweeping $k$ from 2 to 8 and plotting the inertia (within-cluster sum of squared distances) at each (see Figure~\ref{fig:tahoe-elbow}).
...
Standard k-means on scaled $(x, y)$ converges in 10 iterations and produces four spatially compact clusters with sizes 305, 230, 223, and 310 fires. The mean arc-length span is $\overline{\text{span}}_{\text{standard}} = 0.2650$, or about 31.3 km of shoreline per cluster on average. This is the number every obstacle-aware variant is measured against.
...
The $\beta$ sweep runs the model at 30 values of $\beta$ from 0.05 to 2.5, recording both the objective $J$ \eqref{eq:objective} and the mean span at each. Figure~\ref{fig:tahoe-basin-sweep} shows the two on a shared $\beta$ axis.
 
\begin{figure}[H]
\centering
\makebox[\textwidth]{\includegraphics[width=1.1\textwidth]{figures/tahoe_basin_beta_sweep.png}}
...
\begin{figure}[H]
\centering
\makebox[\textwidth]{\includegraphics[width=1.1\textwidth]{figures/tahoe_basin_three_methods.png}}
...
The same three clusterings run on the 296-fire subset. Standard k-means converges in 5 iterations with sizes 93, 71, 51, and 81 and a mean span of $0.2246$ (about 26.6 km). Equal weights converges in 9 iterations with sizes 94, 51, 71, 80 and a mean span of $0.2261$ -- slightly \emph{worse} than standard k-means by 0.7\%. A $\beta$ sweep of 30 points from 0.05 to 2.0 selects $\beta = 0.86$, which sits in a wide stable low-$J$ region; refitting produces cluster sizes identical to standard k-means (93, 71, 51, 81) and a mean span of $0.2246$ (26.6 km), recovering standard k-means exactly.
 
Figure~\ref{fig:tahoe-near-compare} shows the three panels.
 
\begin{figure}[H]
\centering
\makebox[\textwidth]{\includegraphics[width=1.2\textwidth]{figures/tahoe_near_three_methods.png}}
...
Now, we switch to Lake Mead, whose multi-arm geometry tells a very different story near the shore.
...
Each of the 844 fires is projected onto the boundary spline, yielding its arc-length position $s \in [0,1]$. The values cover the full range, $0.011$ to $0.988$, so fires touch every stretch of shoreline (see Figure~\ref{fig:mead-s}).
...
\paragraph{Standard k-means (baseline).}
Standard k-means on scaled $(x, y)$ converges in 7 iterations and produces four clusters with sizes 185, 124, 273, and 262 fires. The mean arc-length span is $0.3412$, or about 234.1 km of shoreline per cluster on average. That number is much larger than Tahoe's 31 km, mostly because Mead's perimeter is $\sim 6\times$ longer. Each Mead cluster covers about a third of a $\sim686$ km shoreline.
 
\paragraph{Obstacle-aware at equal weights.}
Adding $s$ as a clustering feature with $\beta = 1$ produces a clustering almost identical to standard k-means: the per-cluster spans and mean are exactly the same ($0.0\%$), and only a single fire shifts between two clusters. At this weighting, the shoreline term is not strong enough to have any meaningful impact on cluster boundaries; the clustering reorganizes only once $\beta$ is tuned higher.
 
\paragraph{Obstacle-aware at optimized $\beta$.}
The $\beta$ sweep runs a 30-point grid from $\beta = 0.05$ to $\beta = 2.0$. Figure~\ref{fig:mead-basin-sweep} shows $J$ and span across the grid.
 
\begin{figure}[H]
\centering
\makebox[\textwidth]{\includegraphics[width=1\textwidth]{figures/mead_basin_beta_sweep.png}}
...
\begin{figure}[H]
\centering
\makebox[\textwidth]{\includegraphics[width=0.8\textwidth]{figures/mead_basin_three_methods.png}}
...
Equal weights converges in 8 iterations with sizes 33, 126, 40, and 15, and a mean span of $0.2204$ (151.2 km). That is a $9.1\%$ improvement over standard k-means -- already substantial, and much better than Tahoe's near-shore equal-weights result, which actually made clusters slightly looser.
 
The $\beta$ sweep shows where the obstacle-aware version finds further room to tighten (Figure~\ref{fig:mead-near-sweep}).
 
\begin{figure}[H]
\centering
\makebox[\textwidth]{\includegraphics[width=1\textwidth]{figures/mead_near_beta_sweep.png}}
...
Refitting at $\beta = 1.19$ gives cluster sizes 34, 126, 14, and 40, and a mean span of $0.1964$ (about 134.8 km). That is a $19.0\%$ improvement over standard k-means, the largest gain of any case in the project. Whether these uneven clusters are useful in practice is a question for a future version (see Section~\ref{sec:method-refine}).
...
\begin{figure}[H]
\centering
\makebox[\textwidth]{\includegraphics[width=.8\textwidth]{figures/mead_near_three_methods.png}}
\caption{Lake Mead near-shore: the standard k-means clustering (top) splits the data unevenly, with Clusters 1 and 3 reaching across the water rather than following one stretch of shore. The optimized obstacle-aware clustering (bottom) pulls Cluster 3 onto a single side, splitting the eastern shoreline cleanly between the northeastern (Cluster 3) and southeastern (Cluster 1) shores.}
...
Boundary processing involves choices that were not systematically varied: the source NHD layer, the Douglas-Peucker simplification tolerance, the choice of cubic spline over alternative smooth curves, and lake-specific edits to the raw polygon all matter for projection and span. Two of these also differ between the lakes -- Tahoe's shoreline came from the small-scale (generalized) NHD layer and Mead's from the large-scale (high-resolution) layer, so the boundaries begin at different native resolutions. A more controlled version would draw both from the same high-resolution layer under a common simplification rule, so that any remaining difference in fidelity reflects the method rather than the data source.
